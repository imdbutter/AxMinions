...
class ButcherMinionType : MinionType("butcher", AxMinionsPlugin.INSTANCE.getResource("minions/butcher.yml")!!) {
    private val extraTargets = hashSetOf<EntityType>()
    ...
    override fun onToolDirty(minion: Minion) {
        ...
        extraTargets.clear()
        getConfig().getStringList("target-types").fastFor {
            val entityType = runCatching { EntityType.valueOf(it.uppercase(Locale.ENGLISH)) }.getOrNull()
            if (entityType != null) {
                extraTargets.add(entityType)
            }
        }
    }
    ...
    override fun run(minion: Minion) {
        ...
        minion.getLocation().world!!.getNearbyEntities(
            ...
            if (!canTarget(it)) return@fastFor
    ...
    private fun canTarget(entity: LivingEntity): Boolean {
        if (entity is Player) return false
        if (NMSHandler.get().isAnimal(entity)) return true
        return extraTargets.contains(entity.type)
    }
}
